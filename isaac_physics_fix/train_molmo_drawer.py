...
# frontal handle grasp (asset now has a synthesized handle-bar collider from the
# converter pass): aim at the bar center, fingers straddle the bar vertically
GRASP_LOCAL = (0.0, 0.015, 0.037)   # handle bar center, slider-local
INWARD_AXIS = (0.0, 0.0, -1.0)  # gripper forward -> -open dir (approach the front)
UP_AXIS = (0.0, 1.0, 0.0)       # gripper finger axis -> drawer up (wrap the bar)
...
class MolmoDrawerEnvCfg(DirectRLEnvCfg):
    ...
    finger_reward_scale = 2.0  # fingers straddle the handle bar vertically
...
class MolmoDrawerEnv(DirectRLEnv):
    ...
    def _setup_scene(self):
        ...
        # in-context occluder: a static "countertop" plate just above the drawer top,
        # exactly like the kitchen — makes the isolated-asset top-hook strategy impossible
        blocker_cfg = sim_utils.CuboidCfg(
            size=(0.43, 0.56, 0.03),
            collision_props=sim_utils.CollisionPropertiesCfg(),
            visual_material=sim_utils.PreviewSurfaceCfg(diffuse_color=(0.45, 0.35, 0.25)),
        )
        blocker_cfg.func("/World/envs/env_0/Blocker", blocker_cfg, translation=(-0.20, 0.0, 0.633))
