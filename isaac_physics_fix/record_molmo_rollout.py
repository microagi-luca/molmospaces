...
# ---------------------- rollout recording (trained policy) ----------------------
import numpy as np
from isaaclab_rl.rsl_rl import RslRlVecEnvWrapper
from rsl_rl.runners import OnPolicyRunner
from isaaclab_tasks.direct.franka_cabinet.agents.rsl_rl_ppo_cfg import FrankaCabinetPPORunnerCfg

CKPT = os.path.expanduser(os.environ.get('CKPT', '~/rl_logs/molmo_drawer/model_399.pt'))
OUT = os.path.expanduser('~/rl_logs/molmo_drawer/rollout.npz')

env_cfg = MolmoDrawerEnvCfg()
env_cfg.scene.num_envs = 1
raw = MolmoDrawerEnv(cfg=env_cfg)
...
print('>>> policy loaded from', CKPT, flush=True)

res = env.get_observations()
obs = res[0] if isinstance(res, tuple) else res
rob_traj, drawer_traj = [], []
with torch.inference_mode():
    for i in range(500):
        actions = policy(obs)
        obs, _, dones, _ = env.step(actions)
        rob_traj.append(raw._robot.data.joint_pos[0].cpu().numpy().copy())
        drawer_traj.append(raw._cabinet.data.joint_pos[0, 0].item())
        if bool(dones[0]):
            print(f'>>> episode ended at step {i} (success-termination or timeout)', flush=True)
            break
np.savez(OUT, robot=np.array(rob_traj), drawer=np.array(drawer_traj),
         names=np.array(raw._robot.joint_names))
print(f'>>> recorded {len(rob_traj)} steps -> {OUT}', flush=True)
print(f'>>> FINAL drawer: {drawer_traj[-1]:.4f} m = {drawer_traj[-1]/DRAWER_TRAVEL*100:.1f}% of travel', flush=True)
print(f'>>> MAX drawer:   {max(drawer_traj):.4f} m = {max(drawer_traj)/DRAWER_TRAVEL*100:.1f}%', flush=True)
os._exit(0)
